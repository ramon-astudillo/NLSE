...
def save_features(in_fnames, out_folder, no_tokenize=False, one_hot=False, seed=1234):
...
def save_embedding(emb_path, pretrained_emb, index_path, UNK=False):
    ...
    with open(emb_path) as fid:
        # Get emb size
        #print "CHAPUZA! for senna"
        #emb_size = 50
        _, emb_size = fid.readline().split()
        # Initialization
        if UNK:
            with open(count_path, "r") as fid:
                wrd_count = cPickle.load(fid)
            w0 = np.sqrt(6./(n_in + n_out))
            E = rng.uniform(low=-w0, high=w0, size=size)
        else:
            #E = np.zeros((int(emb_size), len(wrd2idx)))
            from nlse.nlse import init_W
            rng = np.random.RandomState(1234)
            #E = nlse.init_W((int(emb_size), len(wrd2idx)), rng, init='glorot-tanh')
            E = init_W((int(emb_size), len(wrd2idx)), rng, init=0.001,
                            shared=False)
...
#    def char_dist(word, word_ref):
#        wrlen = len(word_ref)
#        dist = wrlen - len(set(word) & set(word_ref))
#        return dist
#
#    # Write OOEVs down
#    with open('ooevs.txt', 'w') as fid:
#        for wrd in list(ooev_words):
#            fid.write(wrd + '\n')
#    print "OOEV list written in %s" % 'ooevs.txt'
    ...
    # save the ooev list
    ooev_file = os.path.dirname(pretrained_emb) + '/'  \
              + os.path.basename(pretrained_emb).split('.')[0] + '.ooev.pkl'
...
def save_embedding_noooev(emb_path, pretrained_emb, index_path, char_emb_path, mapping_path, UNK=False):
    '''
    Save a matrix of pre-trained embeddings
    '''
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o',
        help='Folder where (-f) indexed dictionary, train/dev/test files or (-e) the embeddings will be stored',
        type=str)
    parser.add_argument('-t', help='Optional folder to store tokenized tweets',
        type=str)
    ...
    if args.f:
        ...
        if args.o:
            ...
            save_features(args.f, args.o, seed=args.s, no_tokenize=args.no_tokenize)
